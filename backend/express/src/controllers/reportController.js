...
function sameService(doc, service) {
    ...
    return String(doc.service) === String(service._id);
}
...
async function generateAllReports() {
    ...
    const [
        ...
        }).select('status severity service createdAt startedAt resolvedAt'),
        Incident.find({
            resolvedAt: { $ne: null, $gte: bounds90.rangeStart, $lt: bounds90.rangeEnd }
        }).select('status startedAt createdAt resolvedAt service'),
        Alert.find({
            createdAt: { $gte: bounds90.rangeStart, $lt: bounds90.rangeEnd }
        }).select('status severity service createdAt'),
    ...
    function snapshotFor(days, service) {
        ...
        if (!snapshotCache.has(key)) {
            ...
            snapshotCache.set(key, analyticsSnapshot(
                ...
                filterByService(alerts, service)
            ));
        }
    ...
    for (const days of [7, 30, 90]) {
        ...
        const context = {
            ...
            alerts: windowDocs(alerts, days, 'createdAt', null),
            services,
            analytics
        };
    ...
    for (const service of services) {
        ...
        const context = {
            ...
            alerts: windowDocs(alerts, 7, 'createdAt', service),
            services,
            analytics
        };
