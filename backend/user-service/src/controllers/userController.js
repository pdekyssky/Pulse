...
import { deleteUserNotifications } from '../clients/notificationServiceClient.js';
import { unassignUser } from '../clients/incidentServiceClient.js';
...
function isSameUser(actorId, user) {
    return typeof actorId === 'number' && actorId === user.id;
}
...
const getUserById = async (req, res) => {
    try {
        ...
        res.status(200).json(toPublicUserIso(user));
    } catch (error) {
        res.status(500).json({
            message: 'Internal server error',
            error: error.message
...
const createUser = async (req, res) => {
    try {
        ...
        res.status(201).json(toPublicUserIso(user));
    } catch (error) {
        if (error.code === 11000) {
            return res.status(400).json({
                message: 'Email already exists'
...
const updateUser = async (req, res) => {
    try {
        ...
        const actingOnSelf = isSameUser(req.userId, user);
...
const deleteUser = async (req, res) => {
    try {
        ...
        const publicUser = toPublicUserIso(user);

        await unassignUser(user.id);
